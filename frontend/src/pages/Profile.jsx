...
const handleApiRequest = useCallback(async(method, getEndPoints, data=null) =>{
    const headers = getAuthHeaders();   // ✅ this is the headers object
    if(Object.keys(headers).length === 0){   // ✅ proper check for "no token"
        navigate("/login");
        return;
    }
    ...
    try {
        ...
        const config = {
            ...
            headers,   // ✅ just use it directly, already correctly formatted
        };
        if(data){
            config.data = data;
        }
        ...
        if(error.response?.status === 401){   // ✅ also fixed: was error.response?.useState
            navigate("/login");
            return;
        }
        throw error;
...
const handleSaveProfile = useCallback(async () => {
    try {
        ...
        if(data){
            ...
            // Call the update function passed from App.jsx via Layout/Outlet
        ...
        <div className={profileStyles.content}>
            <div className={profileStyles.grid}>
                ...
                <div className={profileStyles.card}>
                    ...
                    className={`${profileStyles.buttonPrimary} mt-6 w-full hover:opcity-90`}
                    disabled={loading}
                    >
